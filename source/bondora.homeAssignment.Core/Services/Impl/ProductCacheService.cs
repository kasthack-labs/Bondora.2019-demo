...
namespace bondora.homeAssignment.Core.Services.Impl
{
    public class ProductCacheService : IProductCacheService
    {
        ...
        public async Task SetProducts(IEnumerable<ProductContract> products)
        {
            ...
            await this.cacheService.Set(this.GetProductsCacheKey(), products);
        }

        public async Task<IEnumerable<ProductContract>> GetProducts()
        {
            var ret = await this.cacheService.Get<IEnumerable<ProductContract>>(this.GetProductsCacheKey());
            if (ret == null)
            {
        ...
        public async Task ResetProducts() => await this.cacheService.Remove(this.GetProductsCacheKey());

        public async Task SetProduct(ProductContract product)
        {
            this.logger.LogDebug($"Caching product #{product?.Id}");
            await this.cacheService.Set(this.GetProductCacheKey(product?.Id), product);
        }

        public async Task<ProductContract> GetProduct(long id)
        {
            var ret = await this.cacheService.Get<ProductContract>(this.GetProductCacheKey(id));
            if (ret == null)
            {
